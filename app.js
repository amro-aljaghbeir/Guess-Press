...
function updateClassicModal(remaining) {
  ...
  container.innerHTML = `
    ...
    <div class="answer"><strong>Answer:</strong> ${classic.reveal ? question.answer : "(hidden)"}</div>
    <p style="text-align:center;">Answering: <strong>${state.teams[classic.answeringTeam]}</strong></p>
  ...
  if (!classic.reveal) {
    ...
    nextBtn.textContent = classic.phase === "team1" ? "Switch Team" : "Reveal Answer";
...
function updateAuctionModal(bet, question, topic, difficulty, remaining) {
  ...
  container.innerHTML = `
    ...
    <div class="timer">${remaining}s</div>
    <div class="answer"><strong>Answer:</strong> ${question.answer}</div>
    <p style="text-align:center;">Bet: ${bet}</p>
